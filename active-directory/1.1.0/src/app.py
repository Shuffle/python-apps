...
class ActiveDirectory(AppBase):
    ...
    def __ldap_connection(self, server, domain, login_user, password, use_ssl):
        use_SSL = False if use_ssl.lower() == "false" else False
        login_dn = domain + "\\" + login_user

        s = Server(server,  use_ssl=use_SSL)
    ...
    def user_attributes(
        ...
        c = self.__ldap_connection(server, domain, login_user, password, use_ssl)
    ...
    def set_password(
        ...
        if new_password != repeat_password:
            ...
            c = self.__ldap_connection(
                server, domain, login_user, password, use_ssl
            )

            result = json.loads( self.user_attributes( server, domain, login_user, password, base_dn, use_ssl, samaccountname, search_base,))
    ...
    def lock_user(self,server,domain,login_user,password,base_dn,use_ssl,samaccountname,search_base):
    ...
    def unlock_user(self,server,domain,login_user,password,base_dn,use_ssl,samaccountname,search_base):
    ...
    def change_user_password_at_next_login(self,server,domain,login_user,password,base_dn,use_ssl,samaccountname,search_base,new_user_password,repeat_new_user_password):
