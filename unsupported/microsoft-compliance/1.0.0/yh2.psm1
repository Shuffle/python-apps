...
# EXO params end
    ...
    process {
        ...
        try
        {
            ...
            Write-Verbose ("UPN: " + $UserPrincipalName.Value)
            Write-Verbose "ModuleVersion: $moduleVersion"
						...
						Write-Verbose("Got connectioncontext " + $ConnectionContext)

						Write-Verbose "Context done"
            ...
            if ($UseRPSSession -eq $true)
            {
                ...
                $ModulePath = [System.IO.Path]::Combine($PSScriptRoot, $ExoPowershellModule);
								...
								Write-Verbose("Starting import of " + $ModulePath)
                Import-Module $ModulePath;
								Write-Verbose "Imported all"
								#cat $ModulePath

                $global:_EXO_ModulePath = $ModulePath;

                $PSSession = New-ExoPSSession -ConnectionContext $ConnectionContext
                #$PSSession = New-PSSession -ConnectionContext $ConnectionContext
								Write-Verbose("Finished PSSession setup before catch")
            ...
            else
            {
                # Download the new web based EXOModule
								Write-Verbose("Pre exomodule: " + $ConnectionContext)
								#Get-Help New-EXOModule 
								#Get-Item New-EXOModule 

                #$ImportedModule = New-EXOModule -ConnectionContext $ConnectionContext
								(Get-Module -ListAvailable New-exomodule*).path
								Write-Verbose "DOne with exomod"
								$ImportedModule = New-exomodule -ConnectionContext $ConnectionContext
                if ($null -ne $ImportedModule)
                {
										Write-Verbose "Got imported module - null"
                ...
                {
										Write-Verbose "Got imported module error - run again?"
                    throw "Module could not be correctly formed. Please run Connect-ExchangeOnline again."
                }
            }

						Write-Verbose "Pre error reporting"
