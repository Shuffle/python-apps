...
class snort3(AppBase):
    ...
    async def create_snort_file(self, file_ref):
        ...
        target_dir = "/usr/local/etc/snort"
    ...
    async def simple_analyze_file(self, config_file, rules_file, pcap_file):
        ...
        cmd = ' '.join(["snort", "-c", config_path, "-R", rules_path, "-r", pcap_path, "-A", "alert_fast"])
        print('Executing the following command: {}'.format(cmd))
        result = subprocess.run(["snort", "-c", config_path, "-R", rules_path, "-r", pcap_path, "-A", "alert_fast"], capture_output=True, text=True)
        alerts = []
